# Table of contents

* [Overview](README.md)

## Administration Plugins

* [Catalogue Poller](administration-plugins/catalogue-poller.md)

## Step Plugins

* [Automatic setting of the representative](step-plugins/representativecreation.md)
* [Electronic Publications](step-plugins/intranda_step_pdfupload.md)
* [LayoutWizzard](step-plugins/layoutwizzard/README.md)
  * [Using the plugin](step-plugins/layoutwizzard/01_use/README.md)
    * [Preview](step-plugins/layoutwizzard/01_use/01_preview/README.md)
      * [Image area](step-plugins/layoutwizzard/01_use/01_preview/01_image.md)
      * [Display and navigation options](step-plugins/layoutwizzard/01_use/01_preview/02_navigation.md)
    * [Single page view](step-plugins/layoutwizzard/01_use/02_imageview/README.md)
      * [Folder and file options](step-plugins/layoutwizzard/01_use/02_imageview/01_folders.md)
      * [Current image](step-plugins/layoutwizzard/01_use/02_imageview/02_image.md)
      * [General settings](step-plugins/layoutwizzard/01_use/02_imageview/03_general.md)
      * [File list](step-plugins/layoutwizzard/01_use/02_imageview/04_filelist.md)
      * [Save view](step-plugins/layoutwizzard/01_use/02_imageview/05_saveview.md)
      * [Working steps](step-plugins/layoutwizzard/01_use/02_imageview/06_steps.md)
      * [Ausgewählter Analyseschritt](step-plugins/layoutwizzard/01_use/02_imageview/07_analysestep.md)
      * [Global cutting options](step-plugins/layoutwizzard/01_use/02_imageview/08_cropping.md)
  * [Technical details](step-plugins/layoutwizzard/02_details/README.md)
    * [Installation](step-plugins/layoutwizzard/02_details/01_installation.md)
    * [Configuration of the LayoutWizzard](step-plugins/layoutwizzard/02_details/02_configuration.md)
    * [Configuration of the user interface](step-plugins/layoutwizzard/02_details/03_ui.md)
    * [Workflow](step-plugins/layoutwizzard/02_details/04_workflow.md)
* [OCR page selection](step-plugins/intranda_step_ocrselector.md)
* [OCR execution with mixed fonts](step-plugins/intranda_step_mixedocr.md)
* [Writing XMP metadata to image files](step-plugins/intranda_step_xmp.md)
* [Split PDFs, Extract Full Text and Read Table of Contents](step-plugins/intranda_step_pdf-extraction.md)
* [Changing the workflow based on process properties](step-plugins/intranda_step_changeworkflow.md)
* [Display of metadata in a task](step-plugins/intranda_step_displaymetadata.md)
* [Automatic pagination based on file names](step-plugins/intranda_step_imagename_analyse.md)
* [EWIG Long term archiving](step-plugins/ewig.md)
* [Renaming files](step-plugins/intranda_step-rename-files.md)
* [Tif-Validation](step-plugins/intranda_step_jhove-validation.md)
* [Metadata validation within a task](step-plugins/intranda_step_metadata_yerusha_validation.md)
* [Automatic enrichment of metadata from own vocabularies](step-plugins/intranda_step_metadata_replacement.md)
* [Quality control of images](step-plugins/intranda_step_imageqa.md)

## Opac Plugins

* [EAD data transfer](opac-plugins/goobi-plugin-opac-ead.md)
* [Generic XML Import](opac-plugins/generic-xml-import.md)

## Import Plugins

* [Import for journal articles from an Endnote Export](import-plugins/intranda_import_endnote.md)
* [Importing records from an Excel file](import-plugins/intranda_import_excel_read_headerdata.md)

## Export Plugins

* [Fedora Export](export-plugins/plugin_intranda_export_fedora.md)
* [Fedora Export PROV](export-plugins/plugin_intranda_export_fedora_prov.md)
* [HAAB Export](export-plugins/haabexport.md)

## Workflow plugins

* [Generic import plugin for excel files including validation](workflow-plugins/goobi-plugin-workflow-excelimport.md)
* [Barcode Generator](workflow-plugins/plugin-workflow-barcode-generator.md)

## Other

* [Counterscript Application](other/counterscript-application.md)

