...
### Changing properties
Properties can be added, changed or deleted using the configuration shown in the following example:
...
| Parameter | Explanation |
| :--- | :--- |
| `name` | Define the name of the property to be changed or deleted here. |
| `value` | Specify the value to be assigned to the property here. If it does not yet exist, it is created with this value. However, if it already exists, it is changed to this value. If a property is to be deleted, the "value" parameter does not need to be specified. |
| `delete` | If a property is to be deleted, this parameter must be specified with the value "true". |
